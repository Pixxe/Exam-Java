...
public class Pion extends Piece implements Mouvement {
	...
	//On déclare en private pas en public
	...
	public boolean bouger(Position pos) {
		...
		//Pas d'accolade dans le if, possible car il y a n'y qu'une instruction
		if (temp)
		{
			_1erTour = false;
		}
			...
			for (Piece other : Piece.getBlackPiece()) {
				if (getPosition().getY() - other.getPosition().getY() == -1) {
					if (Math.abs(getPosition().getX() - other.getPosition().getX()) == 1) {
						...
						//on déclare une variable x de type int mais on réutilise other.position.getX(); au lieu de la variable
